        ...
        const evalRes = await send('Runtime.evaluate', {
          expression: `
            new Promise((resolve, reject) => {
              import('/src/main.tsx')
                .then(m => resolve('IMPORTED SUCCESS: ' + Object.keys(m)))
                .catch(err => resolve('IMPORT ERROR: ' + err.stack || err.message || String(err)))
            })
          `,
          awaitPromise: true
        });

        console.log("EVAL IMPORT RESULT:", evalRes);

        const checkRoot = await send('Runtime.evaluate', {
          expression: 'document.getElementById("root").innerHTML'
        });
        console.log("CHECK ROOT AFTER IMPORT:", checkRoot?.result?.value?.length, checkRoot?.result?.value?.slice(0, 150));
