  ...
  http.get(`http://127.0.0.1:${port}/json`, async (res) => {
    ...
    res.on('end', async () => {
      ...
      ws.onopen = async () => {
        ...
        // Scroll down to clubs and zones
        await send('Runtime.evaluate', { expression: 'window.scrollTo(0, 550)' });
        await new Promise(r => setTimeout(r, 1000));

        console.log("Capturing clubs & zones view...");
        ...
        if (shot && shot.data) {
          ...
          fs.writeFileSync('C:/Users/admin/.gemini/antigravity/brain/23f01a5d-861a-4e45-94bc-9090a5b438cf/clubs_live.png', buf);
          console.log("Saved clubs_live.png (" + (buf.length / 1024).toFixed(1) + " KB)");
        }
